...
namespace Operations.Statistics
{
    ...
    #endregion
    /// <summary>
    /// Implements an Operation which counts the number of words in the text
    /// </summary>
    [OperationDescription("Counts the number of words in the document", MenuName="Proofing Tools")]
    public class WordCountOperation: OperationBase
    {
        #region Constants
        #endregion
        ...
        {
            string[] words = sourceText.Split(new char[]{' '}, StringSplitOptions.RemoveEmptyEntries);
            StatsDialog dlg = new StatsDialog(words.Length);
            dlg.ShowWordCount = true;
            dlg.ShowDialog(ownerWindow);
